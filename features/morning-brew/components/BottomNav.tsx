...
const BottomNav: React.FC<BottomNavProps> = ({ activePath }) => {
  ...
  const navItems = [
    { label: 'Menú', icon: 'restaurant_menu', path: '/' },
    { label: 'Club', icon: 'stars', path: '/loyalty' },
      ...
      <div className="flex justify-between items-center h-16">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <div className={`absolute inset-0 -mx-4 -my-1 rounded-2xl transition-all duration-500 ${
                isActive ? 'bg-primary/5 opacity-100' : 'bg-transparent opacity-0'
              }`}></div>
