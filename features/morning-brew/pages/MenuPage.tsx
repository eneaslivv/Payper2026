...
const MenuPage: React.FC<MenuPageProps> = ({ cart, user, hasActiveOrder, onOpenHub }) => {
  const navigate = useNavigate();
  const [selectedCategory, setSelectedCategory] = useState('Todos');
  const [searchQuery, setSearchQuery] = useState('');

  const itemsMatchingSearch = MENU_ITEMS.filter(item =>
    item.name.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const popularItems = itemsMatchingSearch.filter(item => item.isPopular);
  const otherItems = itemsMatchingSearch.filter(item => 
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex flex-col">
            <h1 className="text-[28px] font-black leading-[0.85] tracking-tighter uppercase text-white italic">
              Morning Brew <br/> <span className="text-primary/90 text-[24px]">Co.</span>
            </h1>
      ...
      <main className="flex-1 flex flex-col gap-6">
        ...
        {!user && (
          <div className="px-6 pt-6">
            <div className="relative group bg-white/[0.02] border border-white/5 rounded-[2.5rem] p-7 overflow-hidden shadow-2xl transition-all hover:bg-white/[0.03] active:scale-[0.98]" onClick={() => navigate('/auth')}>
          ...
          <div className="flex gap-3 px-6 items-center">
            {CATEGORIES.map(cat => (
              <button
                ...
                className={`shrink-0 h-9 px-6 rounded-xl text-[8px] font-black uppercase tracking-[0.2em] transition-all duration-500 border ${
        ...
        <div className="px-6 flex flex-col gap-10 mt-4">
          {selectedCategory === 'Todos' && popularItems.length > 0 && (
            <section>
              ...
              <div className="flex flex-col gap-4">
                {popularItems.map(item => <MinimalCard key={item.id} item={item} onClick={() => navigate(`/product/${item.id}`)} />)}
              </div>
            </section>
          )}
          ...
          <section>
            ...
            <div className="flex flex-col gap-4">
              {otherItems.map(item => <MinimalCard key={item.id} item={item} onClick={() => navigate(`/product/${item.id}`)} />)}
            </div>
          </section>
        </div>
      </main>
...
const MinimalCard: React.FC<{ item: MenuItem, onClick: () => void }> = ({ item, onClick }) => (
  <div 
    ...
    className={`group relative flex items-center gap-5 p-4 rounded-[1.8rem] border transition-all duration-700 overflow-hidden ${
      item.isOutOfStock 
