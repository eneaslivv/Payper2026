...
interface ProductPageProps {
  addToCart: (item: MenuItem, qty: number, customizations: string[], size: string, notes: string) => void;
  user: UserProfile | null;
}

const ProductPage: React.FC<ProductPageProps> = ({ addToCart, user }) => {
          ...
          <div className="flex gap-3">
            {['Chico', 'Grande', 'Venti'].map(s => (
              <label 
                key={s} 
                onClick={() => setSize(s)}
                className={`group relative flex flex-1 cursor-pointer flex-col items-center justify-center rounded-2xl border transition-all duration-500 p-5 ${
                  size === s ? 'border-primary bg-primary/5 shadow-xl' : 'border-white/5 bg-white/[0.01]'
                }`}
              >
