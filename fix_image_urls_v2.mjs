...
async function fixData() {
    ...
    for (const p of products) {
        if (p.image && (p.image.includes('product-images') || p.image.includes(wrongStoreId))) {
            const fixedImage = p.image.replace('product-images', 'products').replace(wrongStoreId, correctStoreId);

            console.log(`Fixing product ${p.name}: ${p.image} -> ${fixedImage}`);
            const { error: updateError } = await supabase
                .from('products')
