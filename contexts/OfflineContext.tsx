...
interface OfflineContextType {
  ...
  createOrder: (order: Order) => Promise<{ orderNumber?: number }>;
...
export const OfflineProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const createOrder = async (order: Order): Promise<{ orderNumber?: number }> => {
    ...
    if (isOnline) {
      try {
        ...
        // Fetch the actual order_number from DB (generated by trigger)
        ...
        try {
          const { data: orderMeta } = await supabase
            ...
            .eq('id', newOrder.id)
            .single();
          orderNumber = (orderMeta as any)?.order_number;
          console.log(`[createOrder] order_number: ${orderNumber}`);
        } catch { /* non-critical */ }
