  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className={`absolute -top-24 left-1/2 -translate-x-1/2 w-[140%] h-64 blur-[110px] opacity-20 transition-all duration-1000 pointer-events-none ${view === 'gate' ? 'bg-primary' : 'bg-blue-500'}`}></div>
          ...
          {view === 'gate' && (
            <div className="flex flex-col items-center w-full text-center">
              ...
              <div className="relative mb-12 group">
                <div className="absolute inset-0 bg-primary/20 blur-[35px] rounded-full animate-pulse"></div>
                <div className="w-24 h-24 rounded-[2.2rem] bg-gradient-to-br from-[#122218] to-[#0a140e] flex items-center justify-center relative z-10 border border-white/10 shadow-inner group-hover:scale-110 transition-transform duration-700">
                  <span className="material-symbols-outlined text-primary text-[42px] fill-icon">person</span>
          ...
          {view === 'how-it-works' && (
            <div className="flex flex-col items-center w-full">
              <header className="flex items-center justify-between w-full mb-10">
                ...
                <span className="text-[9px] font-black uppercase tracking-[0.4em] text-primary italic">Beneficios Club</span>
        ...
        <div className="mt-auto w-full flex flex-col gap-4">
          <button 
            ...
            className="group relative w-full h-[74px] rounded-full bg-primary text-black font-black text-[13px] uppercase tracking-[0.15em] shadow-[0_20px_40px_rgba(54,226,123,0.2)] active:scale-[0.96] transition-all flex items-center justify-center gap-3 overflow-hidden"
          >
          ...
          {view === 'how-it-works' && (
             <button 
              onClick={onClose}
              className="w-full h-12 text-slate-700 font-black text-[9px] uppercase tracking-[0.5em] hover:text-primary transition-colors italic"
            >
...
const StepItem: React.FC<{ icon: string, title: string, desc: string, delay: string }> = ({ icon, title, desc, delay }) => (
  ...
  >
    <div className="w-14 h-14 rounded-[1.4rem] bg-white/[0.03] flex items-center justify-center text-primary shrink-0 border border-white/[0.03] shadow-lg">
