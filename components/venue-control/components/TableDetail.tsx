...
const TableDetail: React.FC<TableDetailProps> = ({ table, mode, onClose, onUpdateStatus, onUpdateProperty }) => {
  ...
  const handleCloseTable = async () => {
    ...
    const canForceClose = profile?.role === 'store_owner' || profile?.role === 'super_admin' || profile?.role === 'admin' || (profile as any).is_admin;
    if (!canForceClose) {
                ...
                <div className="bg-[#0a0a0a] border-t border-zinc-900 p-6 space-y-6 rounded-t-[40px] shadow-[0_-10px_40px_-10px_rgba(0,0,0,0.5)]">
                  ...
                  {table.status === TableStatus.RESERVED ? (
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="bg-indigo-500/[0.07] border border-indigo-500/20 rounded-2xl p-5 space-y-4">
                        <div className="flex items-center justify-between">
                          ...
                          {table.reservedAt && (
                            <p className="text-[9px] text-zinc-600 font-bold">
                              hace {Math.round((Date.now() - table.reservedAt.getTime()) / 60000)} min
                            </p>
                          )}
                        </div>
                        ...
                        {table.reservedFor && (
                          <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full bg-indigo-500/10 flex items-center justify-center text-indigo-400 font-black text-sm uppercase">
                              {table.reservedFor[0]}
                            </div>
                            <div>
                              <p className="text-sm font-black text-white uppercase italic">{table.reservedFor}</p>
                              <p className="text-[8px] text-zinc-600 font-bold">
                                {table.reservationPax || pax} personas
                                {table.reservationCredit && table.reservationCredit > 0
                                  ? ` · $${Number(table.reservationCredit).toLocaleString()} credito`
                                  : ''}
                              </p>
                            </div>
                          </div>
                      ...
                      {/* Share Invitation — persistent invite link */}
                      {table.reservationInviteToken && (
                        <div className="space-y-2">
                          <p className="text-[9px] font-black text-zinc-600 uppercase tracking-widest px-1">Invitar al Cliente</p>
                          <div className="flex gap-2">
                            <button
                              onClick={() => {
                                const inviteUrl = `${window.location.origin}/reserve/${table.reservationInviteToken}`;
                                const msg = encodeURIComponent(
                                  `Te esperamos! Tu mesa esta reservada.\n\n` +
                                  `Mesa: ${table.name}\n` +
                                  `Personas: ${table.reservationPax || pax}\n` +
                                  (table.reservationCredit && table.reservationCredit > 0 ? `Credito: $${Number(table.reservationCredit).toLocaleString()}\n` : '') +
                                  `\nAccede al menu:\n${inviteUrl}`
                                );
                                window.open(`https://wa.me/?text=${msg}`, '_blank');
                              }}
                              className="flex-1 py-3 bg-[#25D366] text-white text-[9px] font-black uppercase tracking-widest rounded-2xl flex items-center justify-center gap-2 hover:scale-[1.02] transition-all"
                            >
                            ...
                            <button
                              onClick={() => {
                                navigator.clipboard.writeText(`${window.location.origin}/reserve/${table.reservationInviteToken}`);
                                addToast('Link copiado', 'success');
                              }}
                              className="px-4 py-3 bg-zinc-900 text-zinc-400 text-[9px] font-black uppercase tracking-widest rounded-2xl border border-zinc-800 hover:text-white transition-all"
                            >
                              Copiar Link
                            </button>
                          </div>
                          <div className="p-2 bg-zinc-900/50 rounded-xl text-[8px] font-mono text-zinc-600 break-all select-all">
                            {window.location.origin}/reserve/{table.reservationInviteToken}
                          </div>
                        </div>
                      )}
