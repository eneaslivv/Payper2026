-- HOTFIX: Restore finalize_order_stock and add recipe_multiplier support
-- This replaces the broken migration
...
BEGIN
    ...
    -- 2. Determinar ubicación de descuento (Fallback al Principal si no se provee)
...
-- Add recipe_multiplier column to product_variants if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'product_variants' AND column_name = 'recipe_multiplier'
    ) THEN
        ALTER TABLE product_variants ADD COLUMN recipe_multiplier NUMERIC DEFAULT 1.0;
    END IF;
END $$;
