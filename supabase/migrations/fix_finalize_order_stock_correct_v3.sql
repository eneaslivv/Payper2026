-- Fix: Auto-detect sellable_type if missing in JSON by checking products table
...
DECLARE
    ...
    v_item_qty NUMERIC;
    v_sellable_type_raw TEXT; -- check raw value
...
BEGIN
    ...
    LOOP
        ...
        ELSE
             -- If missing, check if it exists in PRODUCTS table
             PERFORM 1 FROM products WHERE id = v_sellable_id;
             IF FOUND THEN
                v_sellable_type := 'product';
             ELSE
                v_sellable_type := 'inventory_item';
             END IF;
        END IF;
        ...
        IF v_sellable_type = 'inventory_item' THEN
            ...
            -- A.2 Descuento por Ubicación (FIXED COLUMN item_id AND CONSTRAINT keys)
        ...
        ELSIF v_sellable_type = 'product' THEN
            ...
            LOOP
                ...
                -- Location (FIXED COLUMN item_id AND CONSTRAINT keys)
                IF v_target_location_id IS NOT NULL THEN
                    INSERT INTO inventory_location_stock (item_id, location_id, store_id, closed_units)
