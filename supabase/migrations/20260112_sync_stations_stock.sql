-- Migration: Sync Dispatch Stations with Storage Locations & Update Stock Logic
-- Date: 2026-01-12

-- 1. Ensure 'dispatch_stations' exists and has linkage column
CREATE TABLE IF NOT EXISTS public.dispatch_stations (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    created_at TIMESTAMPTZ DEFAULT NOW(),
    store_id UUID NOT NULL, -- FK typically to stores(id) but enforcing here might break if table exists without it
    name TEXT NOT NULL,
...
-- Add column to link to storage_locations
...
-- 2. Update 'finalize_order_stock' to check station logic
...
BEGIN
    ...
    -- Prioridad 1: p_location_id explícito (si se pasó)
    -- Prioridad 2: Ubicación vinculada a la Estación de Despacho del pedido
    -- Prioridad 3: Depósito Principal (Fallback)
