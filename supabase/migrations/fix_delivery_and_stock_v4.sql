-- FIX: Delivery Ambiguity and Unified Stock Deduction (v4)

-- 1. Drop ambiguous functions and old triggers
DROP FUNCTION IF EXISTS public.finalize_order_stock(uuid);
DROP FUNCTION IF EXISTS public.finalize_order_stock(uuid, uuid);
DROP TRIGGER IF EXISTS trg_finalize_stock ON orders;
DROP FUNCTION IF EXISTS public.finalize_order_stock(); -- Drop trigger function to recreate clean

-- 2. Re-create the Trigger Function (from v3 logic)
...
-- 4. Update RPC confirm_order_delivery to REMOVE explicit call
